...
\begin{itemize}
 ...
 \item As of March 2016, more than 257 users have been granted access to the cluster
\end{itemize}
\end{frame}
...
\#SBATCH \ddash{}cpus-per-task=5
\#SBATCH \ddash{}mem=11000 \#\# max amount of memory per node you require
\#SBATCH \ddash{}error=myprog-\%A\_\%a.err \#\# \%A - filled with jobid. \%a - filled with job arrayid
\#SBATCH \ddash{}output=myprog-\%A\_\%a.out \#\# \%A - filled with jobid. \%a - filled with job arrayid
\#\# Useful for remote notification
\#SBATCH \ddash{}mail-type=BEGIN,END,FAIL,REQUEUE,TIME\_LIMIT\_80
\#SBATCH \ddash{}mail-user=user@test.org
...
[login lus]\$ squeue
JOBID      PARTITION    NAME     USER    ST   TIME    NODES  NODELIST(REASON)
470[1-30]  community.q  example  user99  PD   00:00   1      (resource)
...
\begin{frame}
  ...
  \includegraphics[width=1\textwidth]{images/job_array}
