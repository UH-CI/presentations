...
\title[On-boarding]{HPC On-boarding} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{frame}
	\frametitle{What is a Job Scheduler?}
	...
	\begin{itemize}\footnotesize
        ...
        \item The ability for the user to interact with the schduler to define workflows and/or define job dependencies
        \item[--] Interfaces which help to define workflows and/or job dependencies
        
        \item The ability to automatically manage the scheduling of user jobs
        \item[--] Automatic submission of executions
        
        \item The functionality to allow users to monitor submitted jobs
        \item[--] Interfaces to monitor the executions
        
        \item The capability to control and prioritize the execution order of unrelated jobs
        \item[--] Priorities and/or queues to control the execution order of unrelated jobs
	\end{itemize}
	For the {\craycs}, we utilize the \textbf{S}imple \textbf{L}inux \textbf{U}tility \textbf{R}esource \textbf{M}anager~\\ or simply known as the \emph{SLURM scheduler}
	\numlessfootnotetxt{\tiny \url{https://en.wikipedia.org/wiki/Job_scheduler}}
...
User submitted jobs are assigned a priority using a fairshare algorithm.  Factors such as: runtime, resource usage request, age of job, and amount of compute time a user has used in recent history are all factored in to assigning a priority to a give job   
\end{block}
%The following slides that cover slurm will cover
%\begin{enumerate}
%\item Several useful SLURM commands
%\item Resource partitions
%\item example interactive jobs and batch job scripts
%\end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Partition Details}\tiny
\begin{tabular}{c c c c c c c}
  \toprule                                                                    
\textbf{Partition} & \textbf{Time} & \textbf{Nodes} & \textbf{Priority} & \textbf{Shared} & \textbf{Preempt\\Mode} & \textbf{Mem per CPU} \\
\midrule 
community.q & \begin{tabular}{c c}\textbf{Def}&\textbf{Max}\\00:10:00&3-00:00:00\end{tabular} & \begin{tabular}{c c}\textbf{Min}&\textbf{Max}\\1&20\end{tabular} & 10 & NO & OFF &  \begin{tabular}{c c}\textbf{Def}&\textbf{Max}\\3250&6500\end{tabular}\\
exclusive.q & \begin{tabular}{c  c}\textbf{Def}&\textbf{Max}\\00:10:00&3-00:00:00\end{tabular} &  \begin{tabular}{c  c}\textbf{Min}&\textbf{Max}\\1&20\end{tabular} & 10 & NO & OFF &  \begin{tabular}{c  c}\textbf{Def}&\textbf{Max}\\3250&6500\end{tabular}\\
\bottomrule 
\end{tabular}   


%% \begin{columns}[t]
%%   \begin{column}{.3\textwidth}\begin{semiverbatim}\tiny
%% PartitionName=community.q
%%    DefaultTime=00:10:00
%%    MaxTime=3-00:00:00 
%%    MinNodes=1
%%    MaxNodes=20 
%%    Nodes=prod2-[0003-0085]
%%    Priority=10
%%    Shared=NO 
%%    PreemptMode=OFF
%%    DefMemPerCPU=3250 
...
%% \begin{columns}[t]
%% \begin{column}{.3\textwidth}\begin{semiverbatim}\tiny
%% PartitionName=kill.q
%%    DefaultTime=00:10:00
%%    MaxTime=3-00:00:00
%%    MinNodes=1
%%    MaxNodes=12
%%    Nodes=prod2-[0085-0176]
%%    Priority=10
%%    Shared=NO
%%    PreemptMode=REQUEUE
%%    DefMemPerCPU=3250
...
More details on partition constraints can be viewed by visiting the \href{http://www.hawaii.edu/its/ci/hpc-resources/slurm-partitions/}{{\ci} website} or by using the following command
\begin{itemize}\footnotesize
\item[] \texttt{$[$login \ctilde$]$\$ scontrol show partition $<$partition name$>$}
\item[] Example: \texttt{$[$login \ctilde$]$\$ scontrol show partition community.q}
\end{itemize}   
...
\frametitle{Interactive Job with SLURM}
	...
	$[$login \ctilde$]$\$ srun \ddash{}immediate \ddash{}partition sb.q \ddash{}nodes 1 \ddash{}cpus-per-task 5 \ddash{}tasks-per-node 1 \ddash{}time 0-01:00 \ddash{}pty /bin/bash
	\end{block}
	...
	\begin{block}{Interactive session (with X11)}\tiny
		...
		$[$login \ctilde$]$\$ srun.x11 \ddash{}immediate \ddash{}partition sb.q \ddash{}nodes 1 \ddash{}cpus-per-task 5 \ddash{}tasks-per-node 1 \ddash{}time 0-01:00	~\\
...
\begin{itemize}
	...
	\begin{itemize}\footnotesize
	\item \textbf{Answer:} \textbf{NO!}  User files on the cluster \textbf{\emph{are not backed up}}.  It is up to you as the user to validate and maintain your own backups.  The {\ci} team takes no responsibility for any data that is lost due to human or mechanical error.  
	\end{itemize}
\end{itemize}
\end{frame}
